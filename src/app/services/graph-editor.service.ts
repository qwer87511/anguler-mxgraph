...
import { DialogStrategy } from '../strategy/DialogStrategy';
...
export class GraphEditorService {
  ...
  createComponent(uiComponent, parent?) {
    if(uiComponent['type'] == 'button') {
      this.setStrategy(new ButtonStrategy(uiComponent['geometry']));
    }
      ...
      this.setStrategy(new FormStrategy(uiComponent['geometry']));
    }
    else if(uiComponent['type'] == 'dialog') {
      this.setStrategy(new DialogStrategy(uiComponent['geometry']));
    }
    if(parent == undefined)
